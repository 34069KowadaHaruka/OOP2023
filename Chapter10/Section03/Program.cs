...
namespace Section03 {
    class Program {
        static void Main(string[] args) {
            //NormalSearch(); //Match
            //SearchAll(); //Matches
            //SearchAllWithNextMatch(); //NextMatch
            //SearchAndLinq(); //Cast<>
            CapturingGroup(); //グループ化
        }
        ...
        public static void SearchAll() {
            ...
            foreach (Match match in matches) { //Match型に代入 p.261
        ...
        public static void SearchAllWithNextMatch() {
            ...
            while (match.Success) { //matchから取り出せているとtrue SuccessはIsSuccessじゃないんだ
                Console.WriteLine("Index={0}, Length={1}, Value={2}",
                                  match.Index, match.Length, match.Value);
        ...
        private static void SearchAndLinq() {
            var text = "private List<string> results = new List<string>();";
